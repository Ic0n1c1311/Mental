        ...
        {bookedAppointments.length > 0 ? (
          bookedAppointments.map((appointment, index) => (
            <div className="grid grid-cols-[1fr_2fr] gap-4 sm:flex sm:gap-6 py-2 border-b" key={index}>
              <div>
                <img className="w-32 bg-indigo-50" src={appointment.doctor.image} alt={appointment.doctor.name} />
              </div>
              <div className="flex-1 text-sm text-zinc-600">
                ...
                <p className="text-xs mt-1">
                  <span className="text-sm text-neutral-700 font-medium">Date & Time:</span> {appointment.date.toDateString()} | {appointment.time}
                </p>
              </div>
